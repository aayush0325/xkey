...
static void update_stick()
{
	...
	// X axis — A pulls left, D pulls right; opposing keys cancel
	// A only         → -FULL
	// D only         → +FULL
	// A + D          → 0  (cancel)
	// W + A + S      → -FULL  (W/S don't affect X)
	// W + A + S + D  → 0  (A+D cancel)
	...
	// Y axis — W pulls up, S pulls down; opposing keys cancel
	// W only         → -FULL
	// S only         → +FULL
	// W + S          → 0  (cancel)
	// A + W + D      → -FULL  (A/D don't affect Y)
	// W + A + S + D  → 0  (W+S cancel)
	...
	// Diagonal: both axes non-zero → scale to DIAGONAL to keep vector
	// length ≈ FULL
	if (x != 0 && y != 0) {
	...
	// Only emit if state changed — prevents spam on key-repeat
